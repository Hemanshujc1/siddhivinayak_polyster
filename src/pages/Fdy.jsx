...
const Fdy = () => {
  ...
  const advantages = [
    { title: 'Maximum Tenacity (Strength)', desc: 'Fully drawn polymer chains make FDY one of the strongest polyester yarns. It handles extreme weaving tension without breaking.' },
    { title: 'Silk-Like Smooth Finish', desc: 'Flat, parallel filaments create a perfectly smooth fabric surface prized in sarees, linings, and luxury home textiles.' },
    { title: 'Superior Lustre', desc: 'FDY Bright (BR) is extremely shiny — an ideal, cost-effective replacement for silk in high-gloss apparel and furnishings.' },
    { title: 'Excellent Dimensional Stability', desc: 'Does not stretch under tension, making it perfect as warp yarn in weaving where consistent, controlled tension is critical.' },
    { title: 'Wide Luster Range', desc: 'Available in Bright, Semi Dull, and Full Dull — precise control over the final fabric appearance.' },
    { title: 'Low Boiling Water Shrinkage', desc: 'Maintains dimensions during wet processing, essential for consistent dimensionally-stable woven fabrics.' },
  ];
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="FDY: Smooth, lustrous, high-tenacity polyester yarn for weaving sarees, curtains, and linings. Full specs, advantages, and industry guide." />
      </Helmet>
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-5xl">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex-1">
              ...
              <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-3">FDY — Fully Drawn Yarn</h1>
              <p className="text-blue-200 text-lg mb-6">Smooth • Strong • Lustrous Weaving Yarn</p>
              <div className="flex flex-wrap gap-2 mb-8">
                {['50D – 600D', 'Bright / Semi Dull / Full Dull', 'High Tenacity Grade'].map(t => (
            ...
            <div className="w-full md:w-72 space-y-3 shrink-0">
              <div className="bg-green-500/15 border border-green-400/30 rounded-xl p-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {['Sarees', 'Curtains', 'Suitings', 'Embroidery', 'Warp Yarn', 'Industrial Cord'].map(v => (
              ...
              <div className="bg-red-500/15 border border-red-400/30 rounded-xl p-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {['High stretch needed', 'Socks / hosiery', 'Blankets / fleece'].map(v => (
                    <span key={v} className="bg-white/10 text-white text-xs px-2 py-1 rounded-md">{v}</span>
                  ))}
                </div>
              </div>
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-5xl">
          <div className="flex flex-col lg:flex-row gap-8 items-stretch">
            ...
            <div className="w-full lg:w-40 h-40 rounded-xl overflow-hidden shadow-md border border-gray-200 shrink-0 bg-white">
              <img src="/images/products/fdy-spool.png" alt="FDY Yarn Bobbin" className="w-full h-full object-cover" />
            </div>
            <div className="flex-1">
              <h2 className="text-2xl font-bold text-primary-dark mb-3">What is FDY?</h2>
              <p className="text-gray-700 leading-relaxed">
                Fully Drawn Yarn is a continuous filament where all polymer chains have been stretched to their maximum limit during spinning. This makes it incredibly strong, smooth, and naturally lustrous. Unlike <Link to="/products/dty" className="text-accent font-bold hover:underline">DTY</Link>, FDY filaments lie flat and parallel — creating a silk-like fabric surface.
              </p>
            </div>
            <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm shrink-0 w-full lg:w-80">
              <h3 className="font-bold text-primary-dark mb-3 text-sm uppercase tracking-wider">When should you choose FDY?</h3>
              <ul className="space-y-2">
                {[
                  { label: 'Need smooth, flat fabric', desc: 'Sleek fabrics preferred for sarees & linings.' },
                  { label: 'Need high strength (Tenacity)', desc: 'Handles extreme weaving tension without breaking.' },
                  { label: 'Need high-shine finish', desc: 'Reflects light far better than textured yarns.' },
                ].map(b => (
                  <li key={b.label} className="flex items-start gap-2">
                    <Check className="text-accent mt-0.5 w-3.5 h-3.5 shrink-0" />
                    <div>
                      <div className="font-bold text-gray-800 text-xs">{b.label}</div>
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-5xl">
            ...
            {TABS.map((tab, i) => (
              <button
                ...
                className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-bold whitespace-nowrap transition-all ${
                  activeTab === i ? 'bg-primary-dark text-white shadow' : 'text-gray-600 hover:text-primary-dark'
                }`}
              >
                {tab}
              </button>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-primary-dark to-primary">
                    <th className="py-3 px-4 text-left font-bold text-white text-sm">Property</th>
                    <th className="py-3 px-4 text-left font-bold text-blue-200 text-sm">FDY</th>
                    <th className="py-3 px-4 text-left font-bold text-green-200 text-sm">DTY</th>
                  </tr>
                </thead>
                <tbody>
                  {fdyVsDty.map((row) => (
                    <tr key={row.prop} className="border-b border-gray-100 hover:bg-neutral-light transition-colors">
                      <td className="py-2.5 px-4 text-sm font-semibold text-gray-600">{row.prop}</td>
                      <td className="py-2.5 px-4 text-sm font-medium text-blue-700">{row.fdy}</td>
                      <td className="py-2.5 px-4 text-sm font-medium text-green-700">{row.dty}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {activeTab === 3 && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {['Saree Manufacturers', 'Weaving Mills', 'Curtain Manufacturers', 'Suiting Manufacturers', 'Embroidery Units', 'Home Textile Cos.', 'Industrial Cord Mfrs.', 'Export Houses'].map(b => (
      ...
      <section className="py-10 bg-neutral-light">
        <div className="container mx-auto px-4 md:px-6 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Link to="/products/dty" className="group flex items-center gap-4 p-5 bg-white rounded-xl border border-gray-100 hover:border-accent hover:shadow-md transition-all">
              <div className="w-12 h-12 bg-green-100 text-green-600 rounded-full flex items-center justify-center shrink-0 font-bold group-hover:bg-green-600 group-hover:text-white transition-colors">DTY</div>
              <div className="flex-1">
                <div className="font-bold text-primary-dark group-hover:text-accent transition-colors">DTY (Draw Textured Yarn)</div>
                <div className="text-gray-500 text-sm">Soft, stretchable alternative for knitting.</div>
              </div>
            ...
            <Link to="/products/poy" className="group flex items-center gap-4 p-5 bg-white rounded-xl border border-gray-100 hover:border-accent hover:shadow-md transition-all">
              ...
              <div className="flex-1">
                ...
                <div className="text-gray-500 text-sm">The raw material from which FDY is drawn.</div>
              </div>
              <ArrowRight className="text-gray-300 group-hover:text-accent transition-colors shrink-0" />
            </Link>
          </div>
